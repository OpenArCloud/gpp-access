...
import GeoPoseRequest from "../../request/GeoPoseRequest.js";
...
describe('GeoPoseRequest', () => {
    describe('constructor', () => {
        it('expect provided id and default type', () => {
            ...
            expect(JSON.stringify(request)).to.be.equal(
                `{"id":"${requestId}","timestamp":"${request.timestamp}","type":"${defaultType}"}`)
        });
        ...
        it('expect provided type', () => {
            ...
            expect(JSON.stringify(request)).to.be.equal(
                `{"id":"${requestId}","timestamp":"${request.timestamp}","type":"${geoposeObjectType}"}`)
        })
    });
    ...
    describe('Accessors', () => {
        ...
        it('Timestamp', () => {
            ...
            expect(JSON.stringify(request)).to.be.equal(
                `{"id":"${requestId}","timestamp":"${now}","type":"${defaultType}"}`)
        })
        ...
        it('SensorData', () => {
            ...
            request.addSensorData(
                new Sensor(sensorId, SENSORTYPE.camera), new SensorReading(sensorId, ''));
        ...
        it( 'SensorData unique id', () => {
            ...
            expect(() => request.addSensorData(
                new Sensor(sensorId, SENSORTYPE.camera), new SensorReading(sensorId, ''))).to.throw();
        })
        ...
        it('SensorData json', () => {
            ...
            expect(JSON.stringify(request)).to.be.equal(
                `{${requestJson},"sensors":[${sensorJson}],"sensorReadings":[${readingJson}]}`);
